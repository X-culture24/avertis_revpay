...
const CreateInvoiceScreen: React.FC = () => {
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      // Get company TIN from user or fetch from API
      let companyTin = (user as any)?.company?.tin;
      
      // If TIN not in user object, try to fetch company profile
      if (!companyTin) {
        try {
          const profileResponse = await apiService.get('/company/profile/');
          if (profileResponse.success && profileResponse.data) {
            companyTin = profileResponse.data.tin;
          }
        } catch (error) {
      ...
      if (!companyTin) {
        Alert.alert(
          'Setup Required', 
          'Company information not found. Please complete your profile setup.',
          [
      ...
      // Check if invoice was created (success or partial success with 207 status)
      if (response.success || (response.data && (response.data as any).invoice)) {
        const responseData = response.data as any;
        const kraStatus = responseData?.kra_status || 'unknown';
        
        if (kraStatus === 'confirmed') {
          Alert.alert(
            'Success', 
            `Invoice created and submitted to KRA successfully!\n\nInvoice: ${responseData.invoice?.invoice_no || 'N/A'}\nETR: ${responseData.etr_number || 'N/A'}`,
            [{ text: 'OK', onPress: () => navigation.goBack() }]
          );
        } else {
          // Invoice created but KRA submission failed - still a success for the user
          Alert.alert(
            'Invoice Created', 
            `Invoice ${responseData.invoice?.invoice_no || 'N/A'} created successfully!\n\nKRA submission is pending. The system will retry automatically.`,
            [{ text: 'OK', onPress: () => navigation.goBack() }]
          );
        }
      } else {
